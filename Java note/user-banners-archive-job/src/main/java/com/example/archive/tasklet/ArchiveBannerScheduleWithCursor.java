...
public class ArchiveBannerScheduleWithCursor {

    private static final String ARCHIVE_SQL = """ 
            WITH to_move AS ( 
            SELECT id 
            FROM user_banners_archive 
            WHERE id > ? 
            ORDER BY id 
            LIMIT ? 
            ), 
            moved_rows AS ( 
            DELETE FROM user_banners_archive 
    ...
    public void archiveBannerSchedule() {
        ...
        log.info("[archiveBannerSchedule] Bắt đầu archive user_banners, batchSize={}", batchSizeArchive);
        ...
        try {
            while (true) {
                List<Long> movedIds = jdbcTemplate.queryForList(
                        ARCHIVE_SQL, Long.class, lastId, batchSizeArchive);
                ...
                totalMigrated += movedIds.size();
                batchCount++;

                log.info("[archiveBannerSchedule] Batch #{}: {} dòng, lastId={}, tổng {}.",
                        batchCount, movedIds.size(), lastId, totalMigrated);
